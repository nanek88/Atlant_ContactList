...
def user_details(context, user_details_cache):
  try:
    ...
    if len(user) == 0:
      ...
      cache.__setitem__(GlobalUtil.cacheObj, user_details_cache, details)
...
def add_user(context, first_name, last_name, password, new_user_details_cache):
  try:
    ...
    if len(user) == 0:
      ...
      cache.__setitem__(GlobalUtil.cacheObj, new_user_details_cache, details)
  except Exception as e:
        stopWorkflow(str(e))

@step('I want to login as user {first_name}, {last_name} with password {password} into the system and save details in {logged_user_details_cache}')
def user_login(context, first_name, last_name, password, logged_user_details_cache):
  try:
    email = (f"{first_name}.{last_name}@mail.com")
    ...
    if len(user) == 0:
      ...
      cache.__setitem__(GlobalUtil.cacheObj, logged_user_details_cache, details)
  except Exception as e:
        stopWorkflow(str(e))
...
def add_contact(context, first_name, last_name, phone, street1, street2, city, country, logged_user_details_cache):
  try:
    email = (f"{first_name}.{last_name}@mail.com")
...
def contact_details(context, contact_list_details_cache):
  try:
    ...
    if len(contact) == 0:
      ...
      cache.__setitem__(GlobalUtil.cacheObj, contact_list_details_cache, resource_details)
  except Exception as e:
      stopWorkflow(str(e))
...
def validate_contacts(context, added_contact_details_cache, contact_list_details_cache):
    details_added = cache.__getitem__(GlobalUtil.cacheObj, added_contact_details_cache)   
    details_contact_list = cache.__getitem__(GlobalUtil.cacheObj, contact_list_details_cache)
